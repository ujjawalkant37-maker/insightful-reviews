...
export default function AICompare({ products }: Props) {
...
`;
      ...
      console.error(error);
      setComparison("Unable to generate AI comparison.");
    } finally {
      setLoading(false);
    }
  }, [products]);
  ...
  }, [products.length, compareProducts]);

  if (products.length < 2) {
    return (
      <section className="rounded-3xl border bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <h2 className="text-3xl font-bold">🤖 AI Product Comparison</h2>
        <p className="mt-6 text-gray-500">
          Select at least two products to generate an AI comparison.
        </p>
      </section>
    );
  }
    ...
    <section className="rounded-3xl border bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {loading ? (
        <div className="py-20 text-center">
          ...
          <p className="mt-5">AI is comparing products...</p>
        </div>
      ) : (
